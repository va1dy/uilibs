...
function esp:update()
    for plrName, drawing in next, esp.players do
        ...
        local pass = esp.enabled and esp.checkalive(player) and esp:check(player)
        local character = player.Character
        if not pass or not character or not character.PrimaryPart then
            esp:disable(player)
            continue
        end

        local playerName = LEN(plrName) > esp.maxchar and esp.shortnames and SUB(plrName, 1, esp.maxchar) .. '..' or plrName
        local distance = tostring(FLOOR((character.PrimaryPart.Position - camera.CFrame.Position).Magnitude / 3)) .. 'm'
        ...
        if drawing.arrow.Visible then
            local proj = camera.CFrame:PointToObjectSpace(centerMassPos.Position)
            local ang = ATAN2(proj.Z, proj.X)
            local dir = NEWVEC2(COS(ang), SIN(ang))
            local a = (dir * esp.arrowradius * 0.5) + camera.ViewportSize / 2
        ...
        drawing.chams.ins.Adornee = drawing.chams.ins.Enabled and player.Character or nil
        ...
        if drawing.bar.Visible then
            ...
            drawing.bar_inline.Size = esp:floorvector(NEWVEC2(1, -(biggestY - smallestY + 2) + 3))
        ...
        if drawing.kevlarbar.Visible then
            ...
            drawing.kevlarbar_inline.Size = esp:floorvector(NEWVEC2((biggestX - smallestX), 1))
            drawing.kevlarbar_inline.Position = drawing.kevlarbar.Position
            drawing.kevlarbar_inline.Transparency = transparency
            drawing.kevlarbar_outline.Size = drawing.kevlarbar_inline.Size
        ...
        if drawing.weapon.Visible then
            drawing.weapon.Text = LOWER(character.EquippedTool.Value) or ''
